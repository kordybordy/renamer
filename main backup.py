...
def sanitize_filename_human(s: str) -> str:
    if FILENAME_RULES["replace_slash_only"]:
        return s.replace("/", "_")
    return s
...
def format_case_numbers(cases: list[str]) -> str:
    ...
    if len(cases) == 1:
        return sanitize_filename_human(cases[0])

    first = sanitize_filename_human(cases[0])
    rest = ", ".join(sanitize_filename_human(c) for c in cases[1:])
    return f"{first} ({rest})"

def build_filename(party_list, case_numbers, letter_type: str) -> str:
    """
    EXACT HUMAN FORMAT:
    'Cebrat Krzysztof, Cebrat Sylwia I C 3145_25 Pozew + Postanowienie.pdf'
    """

    # 1. Parties as comma-separated, with spaces
    party_block = ", ".join(party_list) if party_list else "Unknown"

    # 2. Case number
    if not case_numbers:
        case_block = "UNKNOWN"
    elif len(case_numbers) == 1:
        case_block = sanitize_case_number(case_numbers[0])
    else:
        first = sanitize_case_number(case_numbers[0])
        others = ", ".join(sanitize_case_number(c) for c in case_numbers[1:])
        case_block = f"{first} ({others})"

    # 3. Letter type (already Polish human text)
    lt = letter_type or "Unknown"

    filename = f"{party_block} {case_block} {lt}.pdf"
